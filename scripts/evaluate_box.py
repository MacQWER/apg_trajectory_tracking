    ...
    def run_eval(
        ...
        **kwargs
        ...
        return  np.mean(stable) if stable.size > 0 else np.nan, \
                ...
                avg_full, std_full, overall_avg, overall_std
...
def load_model(model_path, epoch=""):
    ...
    net, param_dict = load_model_params(model_path, "model_box", epoch=epoch)
    param_dict["self_play"] = 0
    dataset = QuadDataset(1, **param_dict)

    controller = BoxNetWrapper(net, dataset, **param_dict)
...
if __name__ == "__main__":
    ...
    model_path = os.path.join("trained_models", "box", args.model)
    # MPC
    ...
    # DYNAMICS
    dynamics = (
            BoxDynamics(modified_params=modified_params)
            if DYNAMICS == "box" else SimpleDynamics()
        )
    environment = QuadRotorEnvBase(dynamics, params["dt"])

    # EVALUATOR
    evaluator = BoxEvaluator(controller, environment, test_time=1, **params)
